...
/**
 * Applies post-deflate and pre-inflate buffer transformations
 * described in [rfc7692#section-7.2](https://tools.ietf.org/html/rfc7692#section-7.2)
 */
object CompressionCorrector {

  private val EMPTY_DEFLATE_BLOCK = "000000ffff".decodeHex()
  private const val LAST_OCTETS_TO_REMOVE_AFTER_DEFLATION = 4

  private const val FOUR_OCTETS_TO_ADD_BEFORE_INFLATION = 0x0000ffff
  ...
  fun applyPostDeflate(deflated: Buffer) = deflated.apply {
    if (endsWithEmptyDeflateBlock(this)) {
      val newSize = size - LAST_OCTETS_TO_REMOVE_AFTER_DEFLATION
  ...
  fun applyPreInflate(buffer: Buffer) = buffer.apply {
    writeInt(FOUR_OCTETS_TO_ADD_BEFORE_INFLATION)
  }
